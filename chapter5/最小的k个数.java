...
public class 最小的k个数 {
    public static void main(String[] args) {
        printList(getLeastNumbers1(new int[]{4, 5, 1, 6, 2, 7, 3, 8}, 4));
        System.out.println("---------------------------");
        printList(getLeastNumbers2(new int[]{4, 5, 1, 6, 2, 7, 3, 8}, 4));
        System.out.println("---------------------------");
        printList(getLeastNumbers3(new int[]{4, 5, 1, 6, 2, 7, 3, 8}, 4));
    }
    ...
    private static ArrayList<Integer> getLeastNumbers2(int[] input, int k) {
        if (input == null || input.length == 0 || k < 0 || k > input.length) {
            return new ArrayList<>();
        }
    ...
    // T-C: O(N*logN)
    // S-C: O(1)
    private static ArrayList<Integer> getLeastNumbers3(int[] input, int k) {
        if (input == null || input.length == 0 || k <= 0 || k > input.length) {
            return new ArrayList<>();
        }
        Arrays.sort(input);
